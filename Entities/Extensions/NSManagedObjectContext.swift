...
extension NSManagedObjectContext {
    ...
    public func first<ResultType: NSFetchRequestResult & NSObject>(where predicate: NSPredicate) throws -> ResultType? {
        let request: NSFetchRequest<ResultType> = createRequest(where: predicate)
        
        return try fetch(request).first
    }
    
    public func first<ResultType: NSFetchRequestResult & NSObject & Entity>(withPrimaryKey key: Int64) throws -> ResultType? {
...
extension NSManagedObjectContext {
    ...
    private func createRequest<ResultType: NSFetchRequestResult & NSObject>(where predicate: NSPredicate?, sortDescriptors: [NSSortDescriptor]? = nil) -> NSFetchRequest<ResultType> {
    ...
    private func first<ResultType: NSFetchRequestResult & NSObject & Entity>(withPrimaryKey key: NSObject) throws -> ResultType? {
